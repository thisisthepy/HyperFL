...
class LocalUpdate_PPSGD(object):
    ...
    def local_training(self, local_epoch, round=0):
        ...
        lr_g = 0.1
        optimizer_g = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_g,
        ...
        for ep in range(local_epoch):
            ...
            for it in range(iter_num):
                ...
                #     else:
                #         param.requires_grad = True
                loss_all = self.criterion_dp(output, labels)
                optimizer.zero_grad()
                clipped_grads = {name: torch.zeros_like(param) for name, param in model.named_parameters()}
                parameters_to_clip = [param for name, param in model.named_parameters() if 'fc2' not in name] # PPSGD
                ...
                for name, param in model.named_parameters():
                    # if param.grad != None:
                    clipped_grads[name] /= (loss_all.size()[0])
                for name, param in model.named_parameters():
                    # if param.grad != None:
