...
class LocalUpdate_HyperFL_LPM(object):
    ...
    def local_training(self, local_epoch, round=0):
        ...
        optimizer = torch.optim.SGD(model.fc.parameters(), lr=self.args.lr_g, momentum=0.5, weight_decay=0.0005)
        ...
        for ep in range(epoch_classifier):
            ...
            for it in range(iter_num):
                ...
                # print('update classifier loss', loss)
                loss.backward()
                optimizer.step()
        ...
        for ep in range(local_ep_rep):
            ...
            for it in range(iter_num):
                ...
                grads = torch.autograd.grad(loss, params_to_grads, retain_graph=True)
                ...
                optimizer.step()
                iter_loss.append(loss.item())
                del weights, params_to_grads, grads, hypernetwork_grads, client_embedding_grads
                torch.cuda.empty_cache()
            round_loss.append(sum(iter_loss) / len(iter_loss))
            iter_loss = []
        # ---------------------------------------------------------------------------
